...
import { approveRejectProfilePhotoApi } from "../../../services/channelPartnerService";

export default function ApproveRejectProfilePhoto({
  open,
  onClose,
  popupType,
  userId
  ...
  const label = popupType == 'approve' ? 'Approve Profile Photo' : 'Reject Profile Photo'
  ...
  const {mutate: approveProfilePhoto, isPending: approveLoader} = useMutation({
    mutationFn: approveRejectProfilePhotoApi,
    onSuccess: (res) => {
  ...
  const handleSubmit = () => {
    if (!fieldValue.trim()) {
      ...
      if(popupType == 'approve'){
        approveProfilePhoto({id: userId, payload: {comment:fieldValue, approved: true}})
      }else{
        approveProfilePhoto({id: userId, payload: {comment:fieldValue, approved: false}})
      }
